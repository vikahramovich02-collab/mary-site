...
// Сфера из точек на первом экране: точки живут на поверхности шара,
  ...
  (function () {
    ...
    function size() {
      dpr = Math.min(window.devicePixelRatio || 1, 1.25);   // 2× на ретине — в 2,5 раза больше работы без разницы на глаз
  ...
  // Квадратики в иллюстрациях «Как работаем» качаются только пока карточка в кадре.
  (function () {
    var arts = document.querySelectorAll(".flow__art");
    if (!arts.length || !window.IntersectionObserver) { arts.forEach(function (a) { a.classList.add("is-live"); }); return; }
  ...
  // Лента материалов: листание стрелками, с блокировкой на краях.
  (function () {
    var strip = document.querySelector("[data-posts]");
    var prev = document.querySelector("[data-posts-prev]");
    var next = document.querySelector("[data-posts-next]");
    if (!strip || !prev || !next) return;

    function step() {
      var card = strip.querySelector(".post");
      var gap = parseFloat(getComputedStyle(strip).columnGap || "10") || 10;
    ...
    // плашки-фильтры: показываем только материалы выбранного типа
    var tabs = document.querySelectorAll("[data-posts-tabs] .list-tab");
    tabs.forEach(function (tab) {
      tab.addEventListener("click", function () {
        var key = tab.dataset.filter;
        tabs.forEach(function (t) { t.classList.toggle("is-active", t === tab); });
        strip.querySelectorAll(".post").forEach(function (card) {
          card.hidden = key !== "all" && card.dataset.tag !== key;
        });
  ...
  // Просмотры у материалов — та же формула, что на главной (src/lib/views.js).
  (function () {
    var MONTHS = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"];
    function hash(str) { var h = 2166136261; for (var i = 0; i < str.length; i++) { h ^= str.charCodeAt(i); h = Math.imul(h, 16777619); } return (h >>> 0); }
    function fmt(n) { return n >= 1000 ? (n / 1000).toFixed(n >= 10000 ? 0 : 1).replace(".0", "").replace(".", ",") + " тыс." : String(n); }
    document.querySelectorAll(".post").forEach(function (card) {
      var t = card.querySelector("time"), out = card.querySelector(".post__views b");
      if (!t || !out) return;
      var m = t.textContent.trim().match(/(\d+)\s+(\S+)\s+(\d{4})/); if (!m) return;
      var pub = new Date(+m[3], MONTHS.indexOf(m[2]), +m[1]), now = new Date();
      var days = Math.max(0, (now - pub) / 86400000), seed = hash(card.getAttribute("href") || t.textContent);
      var base = 120 + (seed % 380), rate = 6 + ((seed >>> 8) % 22);
      var today = Math.floor((now.getHours() * 60 + now.getMinutes()) / 1440 * rate);
      out.textContent = fmt(Math.round(base + Math.pow(days, 0.85) * rate * 1.6 + today));
    });
  })();
  ...
  (function () {
    ...
    var clip = modal.querySelector(".vmodal__video");
    ...
    function open() {
      ...
      // ролик стартует вместе с окном, со звуком — пользователь сам нажал.
      // Safari показывал «Ошибка»: если первый запуск не удался (ещё не загружены
      // метаданные или браузер не дал звук), пробуем ещё раз после загрузки,
      // а в крайнем случае — без звука, чтобы обзор всё равно шёл.
      if (clip) {
        clip.currentTime = 0;
        var again = function () {
        ...
        clip.play().catch(function () {
          if (clip.readyState < 2) clip.addEventListener("loadeddata", again, { once: true });
          else again();
        });
      }
    }
    ...
    function hide() {
      if (clip) { clip.pause(); clip.muted = false; }
  ...
  (function () {
    ...
    // согласие на обработку данных — обязательная галочка: без неё заявка не уходит
    var agreeBox = form && form.querySelector("[data-agree]");
    var agree = agreeBox && agreeBox.querySelector("input");
    function ready() {
      if (!submit) return;
    ...
    if (agree) agree.addEventListener("change", function () {
      agreeBox.classList.toggle("is-error", !agree.checked);
      ready();
    });
    ...
    if (form) form.addEventListener("submit", function (e) {
      ...
      if (agree && !agree.checked) { agreeBox.classList.add("is-error"); agree.focus(); return; }
      ...
      window.sendLead(form).then(function () {
        ...
        alert("Не получилось отправить. Напишите нам в Telegram или на ceo@maryrose.by.");
      });
    });
  })();
